...
func TestEmbedNoPaths(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := a.embed(t.Context(), nil, tt.w, tt.d)
...
func TestEmbedRewrite(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			path := newDoc(t, tt.in)
			a := app{stdout: io.Discard, stderr: io.Discard}
			if _, err := a.embed(t.Context(), []string{path}, true, false); err != nil {
...
func TestEmbedDiff(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			foundDiff, err := a.embed(t.Context(), []string{path}, false, true)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
	...
	var out bytes.Buffer
	a := app{stdout: &out, stderr: io.Discard}
	if _, err := a.embed(t.Context(), []string{path}, false, true); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	got, err := os.ReadFile(path)
	if err != nil {
...
func TestEmbedNotMarkdown(t *testing.T) {
	...
	a := app{stdout: io.Discard, stderr: io.Discard}
	_, err := a.embed(t.Context(), []string{path}, false, false)
...
func TestEmbedReportsEveryFailure(t *testing.T) {
	...
	a := app{stdout: io.Discard, stderr: io.Discard}
	_, err := a.embed(t.Context(), paths, false, false)
	if err == nil {
...
func TestRunExitStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := run(t.Context(), tt.args, io.Discard, io.Discard)
			if got != tt.want {
